...
export default function SpeakingGapMatchingPage() {
  ...
  useEffect(() => {
    ...
    async function loadExercise() {
      try {
        ...
        setErrorText("");
        if (!exerciseId) {
          throw new Error("No speaking gap exercise selected.");
        }
        ...
        stateResults.forEach((stateItem) => {
          ...
          const blankKey = blanksById[blankId]?.blank_key;
  ...
  const blanks = useMemo(
    () => (Array.isArray(exercise?.blanks) ? exercise.blanks : []),
    [exercise]
  );
  ...
  const heroTitle = useMemo(() => {
    ...
    return title || `题目 ${exercise?.exercise_base?.external_id || exerciseId || ""}`.trim();
  }, [exercise, exerciseId]);
  ...
  const paragraphs = useMemo(
    () => renderParagraphs(exercise?.content_with_placeholders),
    [exercise]
  );
      ...
      await Promise.all(
        blanks.map((blank) => {
          ...
          return saveSpeakingGapBlankState({
            blank: blank.id,
            answer_payload: { selected_option_key: selectedOptionKey },
            is_correct: selectedOption ? Boolean(selectedOption.is_correct) : false,
          });
        })
      );
    } catch (error) {
      setErrorText(error?.message || "保存状态失败。");
    }
  }
    ...
    <div className="cloze-page">
      <div className="cloze-shell">
        <div className="cloze-topbar">
          <Link to="/modules/exam-preparation/sprechen/gap-matching" className="cloze-topbar__back">
            ← Zurück zu Sprechen
          </Link>
          ...
          {exercise?.exercise_base?.level || exercise?.exercise_base?.difficulty || exercise?.exercise_base?.is_real_exam ? (
            <div className="cloze-hero__badges">
              ...
              {exercise?.exercise_base?.is_real_exam ? (
                <span className="cloze-hero__badge cloze-hero__badge--real">真题</span>
              ) : null}
            </div>
          ) : null}
        </section>
                    ...
                    <span key={blankKey} className="cloze-blank-inline">
                      ...
                      >
                        <button
                          ...
                          onClick={() => setActiveBlankKey(blankKey)}
                          aria-haspopup="dialog"
                          aria-expanded={activeBlankKey === blankKey}
                        >
                          {selectedOption?.option_text || `Lücke ${blank?.blank_number || ""}`}
                        </button>
                      ...
                      <ExerciseOptionSheet
                        ...
                        onClose={() => setActiveBlankKey("")}
                        onSelect={(nextValue) => {
                          if (isChecked) {
                            setIsChecked(false);
                          }
                          setAnswers((previous) => ({ ...previous, [blankKey]: nextValue }));
                        }}
                      />
                      {isChecked ? (
                        <span className={[
        ...
        {isChecked ? (
          <section className="cloze-feedback-list">
            ...
            <SpeakingPracticeRecorder language="zh" />
            {blanks.map((blank) => {
              const options = Array.isArray(blank.options) ? blank.options : [];
              const correctOption = options.find((option) => option.is_correct);
              const selectedOption = options.find(
                (option) => option.option_key === answers[blank.blank_key]
              );
              const isCorrect = Boolean(selectedOption?.is_correct);
              return (
                <article
        ...
        {errorText ? <div className="cloze-error">{errorText}</div> : null}
