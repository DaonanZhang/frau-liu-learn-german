...
SOURCE_DIR="${EXAM_SPEAKING_AUDIO_SOURCE_DIR:-$ROOT_DIR/frontend/public/resources/ExamPreparation/exam_preparation_audio/telc_b1_speaking}"
OBJECT_PREFIX="resources/ExamPreparation/exam_preparation_audio/telc_b1_speaking"
...
if [[ ! -d "$SOURCE_DIR" ]]; then
  echo "Speaking audio directory not found: $SOURCE_DIR" >&2
  exit 1
fi
...
run_target() {
  ...
  echo
  echo "===== Syncing telc_b1_speaking audio to $name COS ====="
  "$SYNC_SCRIPT" "$@" \
...
if run_target \
  ...
  shanghai_status=$?
  echo "Shanghai COS Speaking audio sync failed; Frankfurt COS will still be attempted." >&2
fi
...
if run_target \
  ...
  frankfurt_status=$?
  echo "Frankfurt COS Speaking audio sync failed." >&2
fi

echo
echo "Dual COS Speaking audio sync summary: Shanghai status=$shanghai_status, Frankfurt status=$frankfurt_status"
