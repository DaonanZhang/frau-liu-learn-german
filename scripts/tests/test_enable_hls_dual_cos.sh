...
run_upload() {
  ...
  FAKE_FAIL_BUCKET="${1:-}" \
    "$ROOT_DIR/scripts/enable_hls_5seg.sh" \
      "$MEDIA_DIR" "$MEDIA_DIR" --allow Episode --upload-cos
}
...
[[ "$(grep -c -- '-b frauliu-1335740446 -r ap-shanghai upload' "$FAKE_LOG")" == "3" ]]
[[ "$(grep -c -- '-b frauliu-eu-1335740446 -r eu-frankfurt upload' "$FAKE_LOG")" == "3" ]]
[[ "$(printf '%s\n' "$success_output" | grep -c 'Uploaded \[Shanghai\]:')" == "3" ]]
[[ "$(printf '%s\n' "$success_output" | grep -c 'Uploaded \[Frankfurt\]:')" == "3" ]]
...
printf '%s\n' "$success_output" | grep -q 'COS dual-upload summary: successful=6, failed=0'

: > "$FAKE_LOG"
failure_output="$(run_upload "frauliu-eu-1335740446" 2>&1)"
[[ "$(wc -l < "$FAKE_LOG" | tr -d ' ')" == "6" ]]
[[ "$(printf '%s\n' "$failure_output" | grep -c 'Uploaded \[Shanghai\]:')" == "3" ]]
[[ "$(printf '%s\n' "$failure_output" | grep -c 'COS upload failed \[Frankfurt\]:')" == "3" ]]
printf '%s\n' "$failure_output" | grep -q 'COS dual-upload summary: successful=3, failed=3'

: > "$FAKE_LOG"
env_output="$(run_upload_with_env_credentials 2>&1)"
[[ "$(wc -l < "$FAKE_LOG" | tr -d ' ')" == "6" ]]
printf '%s\n' "$env_output" | grep -q 'COS credentials loaded from environment variables.'
printf '%s\n' "$env_output" | grep -q 'COS dual-upload summary: successful=6, failed=0'
